...
void UcPPU::renderNametableScanline(Layer layer, uint16_t* line, uint8_t screen_y, uint8_t trans) {
  ...
  for (x = bg_pos[layer][0], screen_x = 0; screen_x < width; x++, screen_x++) {
...
void UcPPU::render() {
    ...
    uint32_t shift;
    uint8_t width = driver->getWidth();
    uint8_t height = driver->getHeight();
    ...
    if (sprites_enabled) {
      memset(scanline_sprites, 0xFF, height * UCPPU_SPRITES_PER_SCAN * sizeof(uint8_t));
      ...
      for (index = 0; index < UCPPU_SPRITES; index++) {
        ...
        if (tile_x + 7 < 0 || tile_x >= width) { continue; }
        tile_y = (entry & 0xFF) - spr_offset_y;
        if (tile_y + 7 < 0 || tile_y >= width) { continue; }
        ...
        for (y = 0, screen_y = tile_y; y < 8; y++, screen_y++) {
          ...
          for (ssi = 0; ssi < UCPPU_SPRITES_PER_SCAN; ssi++) {
            if (scanline_sprites[screen_y * UCPPU_SPRITES_PER_SCAN + ssi] == 0xFF) { break; }
          }
          // Long as we didn't walk past the end of the buffer, add the sprite reference
          if (ssi < UCPPU_SPRITES_PER_SCAN) {
            scanline_sprites[screen_y * UCPPU_SPRITES_PER_SCAN + ssi] = index;
          }
        }
      }
    ...
    for (screen_y = 0; screen_y < height; screen_y++) {
      ...
      if (sprites_enabled) {
        ...
        for (ssi = 0; ssi < UCPPU_SPRITES_PER_SCAN; ssi++) {
          index = scanline_sprites[screen_y * UCPPU_SPRITES_PER_SCAN + ssi];
      ...
      driver->transmit(scanline, width);
    }
    driver->endData();
}
...
UcPPU::UcPPU(UcPPUDriver& ppuDriver, const uint32_t* tileTable, uint16_t numTiles, const uint16_t* paletteTable, uint16_t numPalettes) {
    ...
    bg_enabled[fg] = 0;
    sprites_enabled = 0;
    scanline = (uint16_t*) malloc(sizeof(uint16_t) * driver->getWidth());
    scanline_sprites = (uint8_t*) malloc(sizeof(uint8_t) * driver->getHeight() * UCPPU_SPRITES_PER_SCAN);
}
void UcPPU::begin() {
  driver->begin();
}
